...
| **006** | Do stop-losses actually improve risk-adjusted returns? | Across 5,031 days of the NASDAQ Composite, **the edge is the search**: the best of 30 stop rules beats buy-and-hold by **+0.094** Sharpe, and the *same search* on shuffled bars with every trend destroyed beats it by **+0.095** (*p* = 0.43). Picking the winner on history then **loses −0.166** forward, and in-sample rank **anti-predicts** out-of-sample (−0.28). Real findings: **20.6%** of stops gap through their price | [📄 PDF](006-do-stop-losses-work/paper/paper.pdf) · [code](006-do-stop-losses-work/) |
| **007** | Do perpetual-futures prices adjust to scheduled funding payments? *(pre-registered, with holdout)* | Across 431,752 Binance settlements, the basis prices only **6%** of the funding payment at the timestamp (frictionless value rejected, *t* = −105), because Binance settles over a ~1-minute window with no instant to price. Across that window it reprices **45%**, after drifting **42%** the other way in the hour before. Both replicated on **34 unseen contracts** (0.449, −0.422), and **spot moves by half as much**: a derivative's payment schedule moves the underlying | [📄 PDF](007-ex-funding-days/paper/paper.pdf) · [code](007-ex-funding-days/) |
| 008 | *How much history do you need to tell skill from luck?* | *planned* | — |
